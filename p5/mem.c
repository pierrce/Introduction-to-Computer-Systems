 ...
 * MODIFIED BY:  Alec Pierce, Lecture 2
 * *****************************************************************************/
...
void* Mem_Alloc(int size)
{
  ...
  if (list_head != NULL){
    ...
    while (ptr != NULL){
      ...
      int tempSize = ptr->size_status;

      if (tempSize%2 == 0 && (size+(int)sizeof(block_header)) < tempSize){
                       
        if((size+(int)sizeof(block_header))-tempSize < (size+(int)sizeof(block_header))-bestPtr->size_status){
          bestPtr = ptr;
        }
    ...
    bestPtr->next = bestPtr+(size+(int)sizeof(block_header));
    bestPtr->next->size_status = bestPtr->size_status - (size+(int)sizeof(block_header));
    bestPtr->size_status = size;
...
{
  block_header* ptr2 = list_head; 
  block_header* tempPtr = list_head;
  int logic = 0;
  ...
  if(logic == 0){
    ...
    tempPtr->size_status = tempPtr->size_status-1;
  }

  while(ptr2 != NULL) {

    if(ptr2->size_status%2 == 0 && ptr2->next->size_status%2 == 0){

      ptr2->size_status = ptr2->size_status+ptr2->next->size_status+(int)sizeof(block_header);
